...
pub async fn handle_command( bot: Bot, msg: Message, cmd: Command) -> ResponseResult<()> {
    ...
    match cmd {
        Command::Answer(cmd) => {
            ...
            if user.is_won {
                bot.send_message(msg.chat.id, "Ты уже победил сегодня!").await;
            }
            //Если человек попытался угадать больше 5 раз - отказываем.
            if user.attempts >= 5{
                bot.send_message(msg.chat.id, "Извини, но ты уже задал свои 3 вопроса сегодня!\
                Возвращайся завтра и попробуй угадать следуйщего персонажа.").await.unwrap();
                return Ok(());
            }

            if cmd == CONFIG.calendar.try_get_daily_character().unwrap() {
                bot.send_message(msg.chat.id, format!("Да, {}, это именно я, ты победил!", author.mention().unwrap_or(author.clone().first_name)))
                    .await;
        ...
        Command::Question(cmd) => {
            ...
            trace!("Увеличенно количество заданных вопросов на 1 для пользователя : {}", user.id);
